...
export class AudioBridge {
  ...
  private latestRoomNumber: string = '101'
  private latestHotelId: number = 1
  ...
  private startAudioSocketServer() {
    this.tcpServer = net.createServer((socket) => {
      ...
      const audioQueue: { header: Buffer; payload: Buffer; durationMs: number }[] = []
      ...
      socket.on('data', (chunk) => {
        audioBufferQueue = Buffer.concat([audioBufferQueue, chunk as any])
        ...
        while (audioBufferQueue.length >= 3) {
          ...
          if (type === 0x10) { // Audio payload (8kHz SLIN)
            recorder?.addPcm(payload, 8000) // capture guest voice into the call recording
            ...
            getActiveSession(roomNumber, callHotelId).then((session) => {
              if (session) {
                ...
                recorder = new CallRecorder(session.id, roomNumber, callHotelId)
            ...
            if (response.serverContent.modelTurn) {
              ...
              for (const part of parts) {
                if (part.inlineData && part.inlineData.mimeType.startsWith('audio/')) {
                  ...
                  recorder?.addPcm(audio24k, 24000) // capture AI voice into the call recording
      ...
      socket.on('close', () => {
        ...
        if (recorder) {
          ...
          rec.finalize().then((id) => {
            if (id) emitLive({ type: 'recording:new', roomNumber, sessionId: rec.sessionId, recordingId: id, hotelId: callHotelId })
          })
        }
        clearAudioQueue()
        geminiWs.close()
  ...
  public async takeoverCall(guestChannelId: string, staffExtension: string) {
