...
import { aiClient, hotelTools, liveTools, systemInstruction, buildSystemInstruction, toolHandlers } from './ai.js';
...
app.get('/api/requests', async (req, res) => {
    try {
        const hotelId = req.query.hotelId ? parseInt(req.query.hotelId) : 1;
...
app.get('/api/rooms', async (req, res) => {
    try {
        ...
        res.json(rooms.map((r) => ({ ...r, online: isRoomOnline(r.roomNumber) })));
    }
    catch (error) {
        ...
        if (calls) {
            ...
            for (const call of calls) {
                if (call.name) {
                    ...
                    if (handler) {
                        const toolResult = await handler(call.args, hotelId);
...
ami.on('ami_data', (evt) => {
    ...
    if (evt.event === 'PeerStatus') {
        ...
        else if (status.toLowerCase().includes('unregister') ||
